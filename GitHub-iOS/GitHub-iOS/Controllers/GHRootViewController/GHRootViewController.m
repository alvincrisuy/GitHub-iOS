...
- (void)viewDidLoad {
    ...
    NSArray *asciiBackButtonRep = [[GHCommon sharedInstance] asciiBack];
...
- (void)transferRepositoryViewController:(id)sender model:(GHRepositoryModel *)model {
    ...
    GHRepositoryViewController *repositoryViewController = [[GHRepositoryViewController alloc] init];
    repositoryViewController.repository = model;
    
    [self pushTransition:repositoryViewController animated:YES];
    
    repositoryViewController.view.frame = self.contentView.bounds;
}
